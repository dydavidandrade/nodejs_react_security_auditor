...
| 1.4.0 | 2026-05-14 | **Detección contextual de secretos en .env**: si el archivo `.env` está en `.gitignore` genera solo advertencia `low` (buena práctica); si no está ignorado genera `critical`. Nuevos métodos `FileSystem.isGitIgnored()` y `FileSystem.findEnvFiles()`. Gitleaks también degrada a `low` los hallazgos en archivos `.env` gitignoreados. Referencias `process.env.XYZ` en código fuente no generan alarma. |
