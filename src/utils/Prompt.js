  ...
  async getAuthConfig() {
    ...
    if (authType === 'token') {
      const { header, value } = await prompt([
        { type: 'input',    name: 'header', message: t('auth_token_header'), default: 'Authorization' },
        { type: 'password', name: 'value',  message: t('auth_token_value'),  mask: '*' },
      ]);
      return { enabled: true, type: 'token', header, value };
    }

    const { loginUrl, username, password: pwd } = await prompt([
      { type: 'input',    name: 'loginUrl',  message: t('auth_url') },
      { type: 'input',    name: 'username',  message: t('auth_user') },
      { type: 'password', name: 'password',  message: t('auth_pass'), mask: '*' },
    ]);
